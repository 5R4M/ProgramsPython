...
class ReporteBalanceBodega:
    ...
    def obtener_saldo_mes_anterior(self, codigo_insumo, fecha_corte):
        ...
        """
        try:
            from src.database.db_manager import obtener_movimientos_balance
    ...
    def setup_ui(self):
        ...
        self.frame_principal_container, self.frame_principal = self.create_titled_frame(main_container, "Filtros de Reporte Balance de Bodega")
        ...
        # Ajusta los anchos de los combobox para que sean más amplios y se ajusten mejor
        ttk.Label(self.frame_ubicacion_content, text="Área:", background="white").grid(row=0, column=0, padx=5, sticky='w')
        self.area_var = tk.StringVar()
        self.combo_area = AutocompleteCombobox(self.frame_ubicacion_content, textvariable=self.area_var, state="normal", width=30)
        self.combo_area.grid(row=0, column=1, padx=5, sticky='ew')

        ttk.Label(self.frame_ubicacion_content, text="Distrito:", background="white").grid(row=0, column=2, padx=5, sticky='w')
        self.distrito_var = tk.StringVar()
        self.combo_distrito = AutocompleteCombobox(self.frame_ubicacion_content, textvariable=self.distrito_var, state="normal", width=30)
        ...
        try:
            ...
            with pd.ExcelWriter(full_path, engine='xlsxwriter') as writer:
                ...
                for hoja_num in range(0, total_movimientos, filas_por_hoja):
                    ...
                    # Cambiar merge_range a write para evitar warning en celdas individuales
