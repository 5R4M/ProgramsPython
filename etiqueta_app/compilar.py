"""
GENERADOR DE EJECUTABLES - VERSIÓN SIMPLIFICADA
Solo genera los 3 programas que realmente necesitas
"""
...
def generar_ejecutable(script, nombre_exe):
    ...
    try:
        ...
        if e.stderr:
            print("\nDetalles del error:")
            print(e.stderr[:500])  # Mostrar primeros 500 caracteres
        return False
...
def main():
    """Función principal"""
    print("="*70)
    print("🔨 GENERADOR DE EJECUTABLES")
    print("="*70)
    ...
    programas = [
        {
            "script": "generar_qr.py",
            "nombre": "GeneradorQR",
            "descripcion": "Generador de QR individuales"
        },
        {
            "script": "generar_reportes_qr.py",
            "nombre": "GeneradorQR_Reportes",
            "descripcion": "Generador de QR de reportes (Rojo, Amarillo, Verde)"
        },
        {
            "script": "sincronizador_automatico.py",
            "nombre": "Sincronizador",
            "descripcion": "Sincronizador automático por tiempo"
        }
    ]
    ...
    if exitosos and not fallidos:
        ...
        print("\n📦 Archivos generados:")
        print("   1. GeneradorQR.exe")
        print("      → Genera QR individuales para cada medicamento")
        print("      → Lee tu Excel y crea códigos QR con URLs")
        
        print("\n   2. GeneradorQR_Reportes.exe")
        print("      → Genera 3 QR especiales (Rojo, Amarillo, Verde)")
        print("      → Cada QR muestra un reporte filtrado")
        
        print("\n   3. Sincronizador.exe")
        print("      → Sincroniza tu Excel automáticamente cada hora")
        print("      → Configurable: cada 30min, 1h, 2h, etc.")
        ...
        print("   2. Copia los 3 archivos .exe donde los necesites")
        print("   3. Ejecuta cualquier .exe directamente")
        ...
        print("   3. Instala dependencias: pip install pandas qrcode reportlab requests psutil schedule")
